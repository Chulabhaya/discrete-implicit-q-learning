...
from common.models import (
    DiscreteValueDiscreteObs,
    DiscreteCriticDiscreteObs,
    DiscreteActorDiscreteObs,
)
...
if __name__ == "__main__":
    ...
    vf1 = DiscreteValueDiscreteObs(env).to(device)
    ...
    for global_step in range(start_global_step, args.total_timesteps):
        ...
        with torch.no_grad():
            ...
            qf1_target_values = qf1_target(observations)
            qf2_target_values = qf2_target(observations)
            qf1_target_a_values = qf1_target_values.gather(1, actions)
            qf2_target_a_values = qf2_target_values.gather(1, actions)
            min_qf_target_values = torch.min(qf1_target_a_values, qf2_target_a_values)

        # calculate value function predictions
        v_pred_values = vf1(observations)
        ...
        vf_loss = (expectile_weight * (value_diff**2)).mean()
        ...
        # calculate TD target
        v_pred_next_values = vf1(next_observations).detach()
        ...
        if global_step % args.policy_frequency == 0:  # TD 3 Delayed update support
            for _ in range(
                ...
                log_action_probs = actor.get_actions(observations, actions.squeeze())

                # calculate policy loss (eq. 7 in IQL paper)
                actor_loss = (-log_action_probs * exp_advantages).mean()
